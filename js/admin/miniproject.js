...
export function createAdminMiniProjectModule(app) {
    return {
        ...
        async renderAdminFirstReviewScheduleContent(teamId, scheduleData, allModules, allBacklogs, backlogToModule, completedBacklogIds = [], imageLinks = []) {
            ...
            try {
                    ...
                    sortedModules.forEach((scheduleModule, moduleIndex) => {
                        ...
                        html += `
                            <div class="admin-module-card" 
                                 ...
                                 ondragstart="app.handleAdminModuleDragStart(event, '${scheduleModule.moduleId}', '${teamId}', 'first')"
                                 ondragover="app.handleAdminModuleDragOver(event)"
                                 ondrop="app.handleAdminModuleDrop(event, '${scheduleModule.moduleId}', '${teamId}', 'first')"
                                ...
                                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.75rem;">
                                    ...
                                    <button type="button" class="btn btn-primary btn-sm" onclick="app.showAdminAddBacklogModal('${teamId}', '${scheduleModule.moduleId}', 'first')" style="padding: 0.3rem 0.6rem; font-size: 0.75rem;">
                                ...
                                <div style="display: flex; gap: 0.75rem; margin-bottom: 0.75rem; align-items: end;">
                                    <div style="flex: 1;">
                                        ...
                                        <input type="date" class="form-input" value="${scheduleModule.startDate || ''}" 
                                               onchange="app.updateAdminModuleDate('${teamId}', '${scheduleModule.moduleId}', 'startDate', this.value, 'first')"
                                    ...
                                    <div style="flex: 1;">
                                        ...
                                        <input type="date" class="form-input" value="${scheduleModule.endDate || ''}" 
                                               onchange="app.updateAdminModuleDate('${teamId}', '${scheduleModule.moduleId}', 'endDate', this.value, 'first')"
                                               style="width: 100%; padding: 0.4rem; font-size: 0.8rem; border: 1px solid var(--border-color); border-radius: 4px;">
                                    </div>
                                </div>
                                ...
                                <div id="admin-module-backlogs-${scheduleModule.moduleId}" 
                                    ...
                                    ${sortedBacklogs.map(backlog => {
                                        ...
                                        return app.renderAdminBacklogItem(backlog, backlogSchedule, scheduleModule.moduleId, teamId, isCompleted, 'first');
                                    }).join('')}
                                </div>
                            </div>
                        `;
                ...
                if (scheduleData.standaloneBacklogs && scheduleData.standaloneBacklogs.length > 0) {
                    ...
                    html += '<button type="button" class="btn btn-primary btn-sm" onclick="app.showAdminAddBacklogModal(\'' + teamId + '\', null, \'first\')" style="margin-bottom: 0.75rem; padding: 0.3rem 0.6rem; font-size: 0.75rem;">';
                    ...
                    standaloneBacklogs.forEach(backlog => {
                        ...
                        if (backlogSchedule) {
                            const isCompleted = completedBacklogIds.includes(String(backlog.id));
                            html += app.renderAdminBacklogItem(backlog, backlogSchedule, null, teamId, isCompleted, 'first');
                        }
                    });
                    ...
                    html += '<div class="admin-card" style="padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 6px; background: #fafbfc;">';
                    html += '<p class="empty-state" style="font-size: 0.85rem; margin: 0;">No standalone backlogs. <button type="button" class="btn btn-primary btn-sm" onclick="app.showAdminAddBacklogModal(\'' + teamId + '\', null, \'first\')" style="padding: 0.3rem 0.6rem; font-size: 0.75rem; margin-left: 0.5rem;">Add One</button></p>';
                    html += '</div>';
                }
        ...
        renderAdminBacklogItem(backlog, backlogSchedule, moduleId, teamId, isCompleted = false, sprint = 'first') {
            ...
            const sprintParam = sprint === 'second' ? `, 'second'` : '';
            ...
            return `
                <div class="admin-backlog-item" 
                     ...
                     ondragstart="app.handleAdminBacklogDragStart(event, '${backlog.id}', '${moduleId || 'standalone'}', '${teamId}'${sprintParam})"
                     ondragover="app.handleAdminBacklogDragOver(event)"
                     ondrop="app.handleAdminBacklogDrop(event, '${backlog.id}', '${moduleId || 'standalone'}', '${teamId}'${sprintParam})"
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem; gap: 0.5rem;">
                        ...
                        <div style="display: flex; gap: 0.3rem; flex-shrink: 0;">
                            <button type="button" class="btn btn-secondary btn-sm" onclick="app.editAdminBacklog('${backlog.id}', '${moduleId || ''}', '${teamId}'${sprintParam})" style="padding: 0.3rem 0.5rem; font-size: 0.7rem; min-width: auto;">
                                <i class="fas fa-edit" style="font-size: 0.7rem;"></i>
                            </button>
                            <button type="button" class="btn btn-danger btn-sm" onclick="app.deleteAdminBacklog('${backlog.id}', '${moduleId || ''}', '${teamId}'${sprintParam})" style="padding: 0.3rem 0.5rem; font-size: 0.7rem; min-width: auto;">
                    ...
                    <div style="display: flex; gap: 0.75rem; align-items: end;">
                        <div style="flex: 1;">
                            ...
                            <input type="date" class="form-input" value="${startDate}" 
                                   onchange="app.updateAdminBacklogDate('${backlog.id}', '${moduleId || ''}', '${teamId}', 'startDate', this.value${sprintParam})"
                        ...
                        <div style="flex: 1;">
                            ...
                            <input type="date" class="form-input" value="${endDate}" 
                                   onchange="app.updateAdminBacklogDate('${backlog.id}', '${moduleId || ''}', '${teamId}', 'endDate', this.value${sprintParam})"
                                   style="width: 100%; padding: 0.35rem; font-size: 0.75rem; border: 1px solid var(--border-color); border-radius: 4px;">
                        </div>
                    </div>
                </div>
            ...
            try {
                ...
                modal.innerHTML = `
                    <div class="modal-content" style="max-width: 95vw; max-height: 95vh; overflow-y: auto; padding: 1rem;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem; padding-bottom: 0.75rem; border-bottom: 1px solid var(--border-color);">
                            <h2 style="margin: 0; font-size: 1.1rem; font-weight: 600; color: var(--text-primary); display: flex; align-items: center; gap: 0.5rem;">
                                <i class="fas fa-check-circle" style="color: #3b82f6; font-size: 1rem;"></i> Second Sprint Schedule - ${escapeHtml(teamName)}
                            </h2>
                            ...
                            <div style="padding: 0.75rem; background: #dbeafe; border-radius: 6px; border-left: 3px solid #3b82f6; margin-bottom: 0.75rem;">
                                ...
                                <div style="display: flex; gap: 0.5rem; margin-bottom: 0.75rem; align-items: center; flex-wrap: wrap;">
                                    ${scheduleData.frozen ? `
                                        ...
                                        <button type="button" class="btn btn-primary btn-sm" onclick="app.generateSecondSprintScheduleContract('${teamId}')" style="padding: 0.4rem 0.8rem; font-size: 0.75rem; background: #6366f1; color: white; border: none;">
                                            <i class="fas fa-file-contract" style="font-size: 0.7rem;"></i> Generate Contract
                                        </button>
                ...
                // Load schedule content
                await app.renderAdminSecondReviewScheduleContent(teamId, scheduleData, allModules, allBacklogs, backlogToModule, completedBacklogIds, imageLinks);
            ...
            try {
                ...
                if (scheduleData.modules && scheduleData.modules.length > 0) {
                    ...
                    sortedModules.forEach((scheduleModule, moduleIndex) => {
                        ...
                        // Get backlogs that are explicitly in the schedule's productBacklogs array
                        // Only show backlogs from secondReviewBacklogs (all backlogs in schedule are from secondReviewBacklogs)
                        const scheduledBacklogIds = new Set((scheduleModule.productBacklogs || []).map(pb => String(pb.backlogId)));
                        ...
                        html += `
                            <div class="admin-module-card" 
                                 ...
                                 draggable="true"
                                 style="margin-bottom: 1rem; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 6px; background: #fafbfc; cursor: move;"
                                 ondragstart="app.handleAdminModuleDragStart(event, '${scheduleModule.moduleId}', '${teamId}', 'second')"
                                 ondragover="app.handleAdminModuleDragOver(event)"
                                 ondrop="app.handleAdminModuleDrop(event, '${scheduleModule.moduleId}', '${teamId}', 'second')"
                                 ondragend="app.handleAdminModuleDragEnd(event)">
                                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.75rem;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                                        <i class="fas fa-grip-vertical" style="color: #9ca3af; cursor: move; font-size: 0.9rem;" title="Drag to reorder module"></i>
                                    ...
                                    <button type="button" class="btn btn-primary btn-sm" onclick="app.showAdminAddBacklogModal('${teamId}', '${scheduleModule.moduleId}', 'second')" style="padding: 0.3rem 0.6rem; font-size: 0.75rem;">
                                        <i class="fas fa-plus" style="font-size: 0.7rem;"></i> Add
                                    </button>
                                </div>
                                ...
                                <div style="display: flex; gap: 0.75rem; margin-bottom: 0.75rem; align-items: end;">
                                    <div style="flex: 1;">
                                        <label style="font-size: 0.7rem; color: var(--text-secondary); font-weight: 500; display: block; margin-bottom: 0.25rem;">Start Date</label>
                                        <input type="date" class="form-input" value="${scheduleModule.startDate || ''}" 
                                               onchange="app.updateAdminModuleDate('${teamId}', '${scheduleModule.moduleId}', 'startDate', this.value, 'second')"
                                               style="width: 100%; padding: 0.4rem; font-size: 0.8rem; border: 1px solid var(--border-color); border-radius: 4px;">
                                    </div>
                                    <div style="flex: 1;">
                                        <label style="font-size: 0.7rem; color: var(--text-secondary); font-weight: 500; display: block; margin-bottom: 0.25rem;">End Date</label>
                                        <input type="date" class="form-input" value="${scheduleModule.endDate || ''}" 
                                               onchange="app.updateAdminModuleDate('${teamId}', '${scheduleModule.moduleId}', 'endDate', this.value, 'second')"
                                               style="width: 100%; padding: 0.4rem; font-size: 0.8rem; border: 1px solid var(--border-color); border-radius: 4px;">
                                    </div>
                                </div>
                                ...
                                <div id="admin-module-backlogs-${scheduleModule.moduleId}" 
                                    ...
                                    ${sortedBacklogs.map(backlog => {
                                        const backlogSchedule = scheduleModule.productBacklogs?.find(pb => pb.backlogId === backlog.id);
                                        const isCompleted = completedBacklogIds.includes(String(backlog.id));
                                        return app.renderAdminBacklogItem(backlog, backlogSchedule, scheduleModule.moduleId, teamId, isCompleted, 'second');
                                    }).join('')}
                                </div>
                            </div>
                        `;
                ...
                // Render standalone backlogs - ONLY from secondReviewBacklogs
                if (scheduleData.standaloneBacklogs && scheduleData.standaloneBacklogs.length > 0) {
                    html += '<h3 style="margin: 1rem 0 0.75rem 0; color: var(--text-primary); font-size: 0.95rem; font-weight: 600; display: flex; align-items: center; gap: 0.5rem;"><i class="fas fa-tasks" style="font-size: 0.9rem; color: #3b82f6;"></i> Standalone Product Backlogs</h3>';
                    html += '<div class="admin-card" style="padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 6px; background: #fafbfc;">';
                    html += '<button type="button" class="btn btn-primary btn-sm" onclick="app.showAdminAddBacklogModal(\'' + teamId + '\', null, \'second\')" style="margin-bottom: 0.75rem; padding: 0.3rem 0.6rem; font-size: 0.75rem;">';
                    html += '<i class="fas fa-plus" style="font-size: 0.7rem;"></i> Add Backlog';
                    html += '</button>';
                    
                    // Only show backlogs from secondReviewBacklogs
                    const standaloneBacklogIds = new Set(scheduleData.standaloneBacklogs.map(b => String(b.backlogId)));
                    const standaloneBacklogs = allBacklogs.filter(b => 
                        b.source === 'secondReview' && 
                        standaloneBacklogIds.has(String(b.id))
                    ...
                    standaloneBacklogs.forEach(backlog => {
                        ...
                        if (backlogSchedule) {
                            const isCompleted = completedBacklogIds.includes(String(backlog.id));
                            html += app.renderAdminBacklogItem(backlog, backlogSchedule, null, teamId, isCompleted, 'second');
                        }
                    });
                    ...
                    html += '<div class="admin-card" style="padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 6px; background: #fafbfc;">';
                    html += '<p class="empty-state" style="font-size: 0.85rem; margin: 0;">No standalone backlogs. <button type="button" class="btn btn-primary btn-sm" onclick="app.showAdminAddBacklogModal(\'' + teamId + '\', null, \'second\')" style="padding: 0.3rem 0.6rem; font-size: 0.75rem; margin-left: 0.5rem;">Add One</button></p>';
                    html += '</div>';
                }
