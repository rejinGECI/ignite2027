...
const app = {
    ...
    showUserStoriesReportOptions() {
        ...
        if (modal) {
            ...
            const typeSelect = document.getElementById('user-stories-report-type');
            if (typeSelect) {
    ...
    async generateUserStoriesReportFromModal() {
        ...
        try {
            await this.generateUserStoriesReport(format, reportType);
        } catch (error) {
    ...
    async generateUserStoriesReport(format, reportType = 'detailed') {
        try {
            // Load all teams
            const teamsQuery = query(collection(window.firebaseDb, 'projectGroups'));
            const teamsSnapshot = await getDocs(teamsQuery);
            ...
            if (format === 'csv') {
                ...
                if (format === 'pdf') {
                    await this.generatePDFReport(reportContent, { groupName: 'User Stories Status' }, { name: reportName });
                } else if (format === 'html') {
                    this.generateHTMLReport(reportContent, { groupName: 'User Stories Status' }, { name: reportName });
                } else if (format === 'docx') {
                    await this.generateDOCXReport(reportContent, { groupName: 'User Stories Status' }, { name: reportName });
                }
            }
        } catch (error) {
            console.error('Error generating user stories report:', error);
    ...
    generateUserStoriesMinimalReportContent(teamsStatus) {
        let html = `
            <!DOCTYPE html>
            <html>
            <head>
                <meta charset="UTF-8">
                <title>User Stories Status Report (Minimal)</title>
                <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;500;600;700;800&family=Lato:wght@400;500;600;700&display=swap" rel="stylesheet">
                <style>
                    body {
                        font-family: 'Lato', sans-serif;
            ...
            <body>
                <div class="report-container">
                    ...
                    <div style="margin-top: 30px;">
                        <h3 style="font-family: 'Montserrat', sans-serif; font-size: 16px; font-weight: 700; color: #1f2937; margin-bottom: 15px;">Team Status</h3>
                        <table class="teams-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 10%;">Stories Count</th>
                                    <th style="width: 17.5%;">Submission Status</th>
                                    <th style="width: 17.5%;">Verification Status</th>
                                </tr>
                            </thead>
                            <tbody>
                                ${teamsStatus.map((team, teamIndex) => {
                                    ...
                                    if (team.needsReverification) {
                                        ...
                                        submissionStatus = '<span class="status-badge status-submitted">⏳ Submitted</span>';
                                        verificationStatus = '<span class="status-badge status-submitted">⏳ Pending</span>';
                                    } else if (team.storiesCount > 0) {
                                        submissionStatus = '<span class="status-badge status-in-progress">📝 In Progress</span>';
                                        verificationStatus = '<span class="status-badge status-not-submitted">Not Submitted</span>';
                                    } else {
                                    ...
                                    return `
                                        <tr>
                                            <td style="text-align: center; color: #4b5563; font-weight: 600;">${teamIndex + 1}</td>
                                            <td style="color: #1f2937; font-weight: 500;">${this.escapeHtml(team.teamName)}</td>
                                            <td style="color: #64748b;">${this.escapeHtml(team.guideName)}</td>
                                            <td style="text-align: center; color: #1f2937; font-weight: 600;">${team.storiesCount}</td>
    ...
    generateUserStoriesCSVReport(teamsStatus, reportType = 'detailed') {
        const csvRows = [];
        const reportTitle = reportType === 'minimal' 
            ? 'User Stories Status Report (Minimal)'
            : 'User Stories Submission Status Report';
        ...
        if (reportType === 'minimal') {
            // Minimal report - just team status table
        ...
        const link = document.createElement('a');
        link.href = url;
        const fileName = reportType === 'minimal' 
            ? `User_Stories_Status_Report_Minimal_${new Date().toISOString().split('T')[0]}.csv`
            : `User_Stories_Status_Report_${new Date().toISOString().split('T')[0]}.csv`;
    ...
    generateUserStoriesJSONReport(teamsStatus, reportType = 'detailed') {
        const reportTitle = reportType === 'minimal' 
            ? 'User Stories Status Report (Minimal)'
            : 'User Stories Submission Status Report';
        ...
        link.href = url;
        const fileName = reportType === 'minimal' 
            ? `User_Stories_Status_Report_Minimal_${new Date().toISOString().split('T')[0]}.json`
            : `User_Stories_Status_Report_${new Date().toISOString().split('T')[0]}.json`;
        link.download = fileName;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    ...
    async loadSchedule() {
        ...
        try {
            ...
            // Load modules and their backlogs
            const modulesQuery = query(
                collection(window.firebaseDb, 'cardSortingModules'),
                where('teamId', '==', team.id)
            );
            ...
            // Build calendar view
            this.renderScheduleCalendar(modules, backlogsByModule, schedules, importantDates);
    ...
    renderScheduleCalendar(modules, backlogsByModule, schedules, importantDates) {
        const container = document.getElementById('schedule-content');
        if (!container) return;
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const isImportantDate = importantDatesMap[dateKey];
            ...
            html += `
                <div class="calendar-day" data-date="${dateKey}" 
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.25rem;">
                        <span style="font-weight: ${isToday ? '700' : '600'}; color: ${isToday ? '#3b82f6' : (isImportantDate ? '#ef4444' : 'var(--text-primary)')}; font-size: 0.9rem;">
                            ${day}
                        </span>
                        ${isImportantDate ? `
                            <span style="background: #ef4444; color: white; padding: 2px 6px; border-radius: 4px; font-size: 0.65rem; font-weight: 600;" title="${this.escapeHtml(isImportantDate.title || 'Important Date')}">
                                <i class="fas fa-star"></i>
                            </span>
                        ` : ''}
                    </div>
    ...
    previousMonth() {
        if (this.currentScheduleMonth === 0) {
            this.currentScheduleMonth = 11;
    ...
    nextMonth() {
        if (this.currentScheduleMonth === 11) {
            this.currentScheduleMonth = 0;
            this.currentScheduleYear++;
        } else {
            this.currentScheduleMonth++;
        }
    ...
    openScheduleModal(dateKey) {
        this.selectedScheduleDate = dateKey;
        const modal = document.getElementById('schedule-task-modal');
        const dateInput = document.getElementById('schedule-date');
        const backlogsList = document.getElementById('schedule-backlogs-list');
        
        if (!modal || !dateInput || !backlogsList) return;
        
        // Set the date
        dateInput.value = dateKey;
        
        // Load available backlogs
        this.loadBacklogsForScheduling(backlogsList);
        
        modal.style.display = 'flex';
    },
    
    async loadBacklogsForScheduling(container) {
        try {
            const team = await this.getUserTeam();
            if (!team) return;
            ...
            modules.forEach(module => {
                ...
                html += `
                    <div style="margin-bottom: 1rem; padding: 1rem; background: white; border-radius: 8px; border: 2px solid ${moduleColor};">
                        <h5 style="margin: 0 0 0.75rem 0; color: ${moduleColor}; font-size: 0.95rem; font-weight: 600;">
                            <i class="fas fa-layer-group"></i> ${this.escapeHtml(module.name)}
                        </h5>
                ...
                moduleBacklogs.forEach(backlog => {
                    ...
                    html += `
                        <label style="display: flex; align-items: start; gap: 0.75rem; padding: 0.75rem; background: ${isScheduled ? '#f0fdf4' : '#f8f9fa'}; border-radius: 6px; cursor: ${isScheduled ? 'not-allowed' : 'pointer'}; border: 1px solid ${isScheduled ? '#10b981' : 'transparent'};">
                            <input type="checkbox" class="schedule-backlog-checkbox" data-backlog-id="${backlog.id}" 
                                   ${isScheduled ? 'checked disabled' : ''} 
                                   onchange="app.toggleBacklogSelection('${backlog.id}')"
                            ...
                            <div style="flex: 1; min-width: 0;">
                                <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem; flex-wrap: wrap;">
                                    <span style="background: ${priorityColor}15; color: ${priorityColor}; padding: 2px 6px; border-radius: 4px; font-size: 0.7rem; font-weight: 600; text-transform: uppercase;">${(backlog.priority || 'medium').toUpperCase()}</span>
                                    ${isScheduled ? `<span style="color: #10b981; font-size: 0.7rem;"><i class="fas fa-calendar-check"></i> Already scheduled</span>` : ''}
                                </div>
                                <div style="font-size: 0.9rem; color: var(--text-primary); font-weight: 500; margin-bottom: 0.25rem;">${this.escapeHtml(backlog.task || 'No task')}</div>
                                ${backlog.acceptanceCriteria ? `
                                    <div style="font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.25rem;">${this.escapeHtml(backlog.acceptanceCriteria)}</div>
                                ` : ''}
                                <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; margin-top: 0.5rem;">
                                    <span style="background: #e0e7ff; color: #3730a3; padding: 4px 8px; border-radius: 6px; font-size: 0.75rem; font-weight: 500;">
                                        <i class="fas fa-user"></i> ${this.escapeHtml(backlog.userName || 'Unknown')}
                                    </span>
                                    <span style="background: #fef3c7; color: #92400e; padding: 4px 8px; border-radius: 6px; font-size: 0.75rem; font-weight: 500; max-width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${this.escapeHtml(backlog.storyText)}">
                                        <i class="fas fa-list-ul"></i> ${this.escapeHtml(backlog.storyText.length > 40 ? backlog.storyText.substring(0, 40) + '...' : backlog.storyText)}
                                    </span>
                                </div>
                            </div>
                        </label>
            ...
            if (html === '') {
                html = '<p class="empty-state">No product backlogs available. Please assign backlogs to modules first.</p>';
            }
            
            container.innerHTML = html;
            this.selectedBacklogsForSchedule = [];
        } catch (error) {
            console.error('Error loading backlogs for scheduling:', error);
            container.innerHTML = '<p class="error-message">Error loading backlogs. Please try again.</p>';
        }
    },
    
    toggleBacklogSelection(backlogId) {
        const checkbox = document.querySelector(`.schedule-backlog-checkbox[data-backlog-id="${backlogId}"]`);
        if (!checkbox || checkbox.disabled) return;
        
        if (checkbox.checked) {
            if (!this.selectedBacklogsForSchedule.includes(backlogId)) {
                this.selectedBacklogsForSchedule.push(backlogId);
            }
        } else {
            this.selectedBacklogsForSchedule = this.selectedBacklogsForSchedule.filter(id => id !== backlogId);
        }
    },
    ...
    async saveSchedule() {
        ...
        if (this.selectedBacklogsForSchedule.length === 0) {
            alert('Please select at least one product backlog to schedule.');
            return;
        }
        ...
        try {
            ...
            if (existingDoc) {
                ...
                const existingBacklogIds = existingData.backlogIds || [];
                
                // Merge with new selections (avoid duplicates)
                const allBacklogIds = [...new Set([...existingBacklogIds, ...this.selectedBacklogsForSchedule])];
